...
export async function loadMemoryInstructions(packageRoot) {
    ...
    if (!info.isFile())
        throw new Error(`Native memory instructions are not a file: ${candidate}`);
    if (info.size > MAX_INSTRUCTIONS_BYTES) {
        throw new Error(`Native memory instructions exceed ${MAX_INSTRUCTIONS_BYTES} bytes: ${candidate}`);
    }
...
export async function registerMemoryInstructions(config, asset, projectDirectory) {
