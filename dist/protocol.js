...
export function decodeResponse(payload) {
    const response = fromBinary(ResponseSchema, payload);
    ...
    push(chunk) {
        ...
        while (offset < this.buffered.byteLength) {
            ...
            if (header.value > this.maxFrameBytes) {
                throw new Error(`Memory response exceeds ${this.maxFrameBytes} bytes (declared ${header.value})`);
            }
            const frameEnd = header.next + header.value;
            if (frameEnd > this.buffered.byteLength)
                break;
...
function encodeValue(input, depth) {
    ...
    if (typeof input === "object") {
        ...
        for (const [key, value] of Object.entries(input)) {
            if (value !== undefined)
                fields[key] = encodeValue(value, depth + 1);
        }
        return create(ValueSchema, {
            kind: {
                case: "objectValue",
...
function decodeValue(input, depth) {
    if (depth > MAX_VALUE_DEPTH) {
    ...
    switch (input.kind.case) {
        ...
        case "objectValue":
            return Object.fromEntries(Object.entries(input.kind.value.fields).map(([key, value]) => [
                key,
